...
async function getUserProfile(userId) {
  // First, try to get user from users table (human users)
  ...
  // If not found in users table, check if this is an AI profile
  if (!user) {
    // Check ai_profiles table using user_id column
    const aiResult = await supabaseQuery(`
      SELECT * FROM ai_profiles WHERE user_id = $1
    `, [userId]);

    const aiProfile = aiResult.data?.[0];
    if (aiProfile) {
      isAI = true;
      // Also get from profiles table for additional data
      const profileResult = await supabaseQuery(`
        SELECT * FROM profiles WHERE id = $1
      `, [userId]);
      const profile = profileResult.data?.[0];

      // Merge AI profile data into user-like structure
      user = {
        id: userId,
        display_name: aiProfile.display_name || profile?.display_name,
        bio: aiProfile.bio || profile?.bio || '',
        seeking: aiProfile.seeking || '',
        ai_model: aiProfile.ai_model || profile?.ai_model,
        is_ai: true,
        profile_visibility: 'public',
        feed_url: aiProfile.feed_url || profile?.feed_url,
        profile_photo_url: profile?.profile_photo_url || null,
        created_at: aiProfile.created_at || profile?.created_at,
        discovery_complete: profile?.discovery_complete || false,
        discovery_answers: profile?.discovery_answers || {}
      };
    }
  }
...
async function getConversation(otherUserId) {
  ...
  // Get messages using supabaseSelect (proxy can't parse complex WHERE)
  try {
    const sent = await supabaseSelect('messages', { where: { sender_id: userId, recipient_id: otherUserId } });
    const received = await supabaseSelect('messages', { where: { sender_id: otherUserId, recipient_id: userId } });
    var allMsgs = [...(sent.data || []), ...(received.data || [])];
    var messagesResult = { data: allMsgs.filter(m => !m.deleted_by || !m.deleted_by.includes(userId)).sort((a, b) => new Date(a.sent_at) - new Date(b.sent_at)) };
  } catch (e) { console.error('getConversation:', e); return []; }
...
async function isBlocked(otherUserId) {
  ...
  try {
    // Check both directions using supabaseSelect
    const result1 = await supabaseSelect('user_blocks', {
      where: { blocker_id: userId, blocked_id: otherUserId },
      limit: 1
    });
    if (result1.data && result1.data.length > 0) return true;

    const result2 = await supabaseSelect('user_blocks', {
      where: { blocker_id: otherUserId, blocked_id: userId },
      limit: 1
    });
    return result2.data && result2.data.length > 0;
  } catch (error) {
    console.error('isBlocked check failed:', error);
    return false;
  }
}
